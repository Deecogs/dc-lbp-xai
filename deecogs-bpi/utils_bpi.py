...
def run_gemini_on_video(encoded_string: str) -> str:
    response = g_client.models.generate_content(
        model="gemini-2.0-flash-001",
        contents=[
            "This video shows a patient showing his/her pain location. Evaluate their movement and return one of the following labels:\n - Lower Back\n - Others\n Only return the label.",
            types.Part.from_bytes(data=encoded_string, mime_type="video/mp4")
        ],
...
def classify_response(user_input: str, expected_options: list[str]) -> str:
    prompt = f"""
        ...
        Given the following valid options: {expected_options}, try classifying their response into one of these or 'others'.
...
def rephrase_question(node_name: str, question: str) -> str:
...
def handle_fallback(question: str, user_message: str):
    ...
    fallback_prompt = f"""
        ...
        IF what they asked is related to triage or healthcare or physiotheraphy: 
